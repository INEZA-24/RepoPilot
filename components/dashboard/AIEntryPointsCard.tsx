      ...
      <div className="repo-header__top">
        <div>
          ...
          <p className="muted" style={{ lineHeight: 1.65, maxWidth: 760 }}>
            Generate up to three evidence-grounded ways to start contributing. NVIDIA is called only after you submit this form.
          </p>
        </div>
        {analysis ? <span className="chip">Source: {analysis.source === "nemotron" ? "Nemotron" : "Heuristic fallback"}</span> : null}
      </div>
      ...
      <form onSubmit={generate} aria-busy={loading}>
        <div className="ai-form-grid">
          <label className="form-field">
            ...
            <select className="select" value={experienceLevel} disabled={loading} onChange={(event) => setExperienceLevel(event.target.value as ContributorProfile["experienceLevel"])}>
          ...
          <label className="form-field">
            ...
            <select className="select" value={preferredContributionType} disabled={loading} onChange={(event) => setPreferredContributionType(event.target.value as ContributorProfile["preferredContributionType"])}>
          ...
          <label className="form-field">
            ...
            <input className="input" value={skills} disabled={loading} onChange={(event) => setSkills(event.target.value)} placeholder="javascript, react" />
            <span className="helper-text">Example: TypeScript, docs, testing</span>
          </label>
        </div>
        <button className="btn btn-primary" disabled={loading} style={{ marginTop: 18, width: "100%" }}>
          {loading ? <><span className="spinner" aria-hidden="true" /> Generating verified entry points…</> : "Generate AI Entry Points"}
        </button>
      </form>
